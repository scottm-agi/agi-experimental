...
class SchedulerTaskCreate(ApiHandler):
    async def process(self, input: Input, request: Request) -> Output:
        ...
        if not project_slug:
            raise ValueError("Project is required for all tasks. Please select or create a project first.")

        try:
            metadata = load_basic_project_data(requested_project_slug)
            project_color = metadata.get("color") or None
        except Exception as exc:
            raise ValueError(f"Project '{requested_project_slug}' not found or could not be loaded. Please ensure the project exists.")

        # Use provided context_id if it exists, otherwise it will default to dedicated context (uuid)
        task_context_id = input.get("context_id")
        if isinstance(task_context_id, str):
            task_context_id = task_context_id.strip() or None
        else:
