...
const model = {
  ...
  // NEW: Copy text to clipboard with feedback
  async copyToClipboard(text) {
    if (!text) return;
    try {
      await navigator.clipboard.writeText(text);
      this.frontendSuccess("Copied to clipboard", "Success", 2);
    } catch (err) {
      console.error("Failed to copy text: ", err);
      this.frontendError("Failed to copy text", "Error", 3);
    }
  },
  ...
  async createNotification(
    ...
    try {
      const response = await API.callJsonApi("/notification_create", {
        type: type,
        message: message,
        title: title,
        detail: detail,
    ...
    if (!frontendOnly) {
      if (this.isConnected()) {
        try {
          ...
          console.log(
            `Backend unavailable for notification, showing as frontend-only: ${error.message || error
            }`
          );
        }
      } else {
