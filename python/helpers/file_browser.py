...
class FileBrowser:
    ...
    def delete_file(self, file_path: str) -> bool:
        ...
        For directories, delegates to files.delete_dir() which provides:
        - 3-attempt retry with delay (handles race conditions with agents/Docker)
        - onerror permission fixing (handles read-only git pack files)
        """
        try:
            ...
            if os.path.exists(full_path):
                if os.path.isfile(full_path):
                    ...
                    # Use robust delete with retry + permission fixing
                    # (bare shutil.rmtree fails on active project dirs — Errno 39)
                    return files.delete_dir(str(full_path))
                return True
