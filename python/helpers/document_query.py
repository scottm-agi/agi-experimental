...
class DocumentQueryStore:
    ...
    def get(agent: Agent):
        ...
        # Use agent's context ID as cache key to persist store across calls
        cache_key = str(id(agent))
        
        if cache_key not in DocumentQueryStore._stores:
            DocumentQueryStore._stores[cache_key] = DocumentQueryStore(agent)
        
        return DocumentQueryStore._stores[cache_key]
...
class DocumentQueryHelper:
    ...
    async def document_qa(
        ...
        # to prevent network-bound document fetches from hanging indefinitely
        index_coros = [self.document_get_content(uri, True) for uri in document_uris]
        if index_coros:
            futures = [asyncio.ensure_future(c) for c in index_coros]
            done, pending = await asyncio.wait(futures, timeout=120.0)
            if pending:
                import logging
                logging.getLogger(__name__).warning(
                    f"document_qa: {len(pending)}/{len(futures)} document indexing tasks "
