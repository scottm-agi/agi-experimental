...
class ImageGet(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        if not path:
            return Response("No path provided", status=400)

        # check if path is within base directory
        # Always normalize path (converts relative to absolute and handles /agix/ prefix)
        original_path = path
        path = files.fix_dev_path(path)
        
        # validate that path is in base dir
        if not files.is_in_base_dir(path):
            from python.helpers.print_style import PrintStyle
            PrintStyle.error(f"Image access denied: {original_path} resolved to {path} outside base {files.get_base_dir()}")
            return Response(f"Access denied: {original_path}", status=403)
        ...
        if file_ext in image_extensions:
            ...
            if runtime.is_development():
                ...
                if files.exists(path):
                    response = send_file(files.get_abs_path(path))
                else:
                    from python.helpers.print_style import PrintStyle
                    PrintStyle.warning(f"Image not found: {path} (original: {original_path})")
                    response = _send_fallback_icon("image")
