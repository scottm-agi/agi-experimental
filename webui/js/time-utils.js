...
/**
 * Get the uptime in minutes since a given UTC ISO string
 * @param {string} utcIsoString - UTC time in ISO format
 * @returns {number} Minutes elapsed since the given time
 */
export function getUptimeMinutes(utcIsoString) {
  if (!utcIsoString) return 0;
  const start = new Date(utcIsoString);
  const now = new Date();
