...
from python.helpers import files, runtime, git_helper
from python.helpers.print_style import PrintStyle


class BackupService:
    """
    Core backup and restore service for AGIX.
    ...
    def __init__(self):
        self.agix_version = self._get_agix_version()
        self.agix_root = files.get_abs_path("")  # Resolved AGIX root

        # Build base paths map for pattern resolution
        self.base_paths = {
            self.agix_root: self.agix_root,
        }
    ...
    def get_default_backup_metadata(self) -> Dict[str, Any]:
        ...
        return {
            "backup_name": f"agix-backup-{timestamp[:10]}",
    ...
    def _get_default_patterns(self) -> str:
        ...
        Only includes AGIX project directory patterns.
        """
        # Ensure paths don't have double slashes
        agent_root = self.agix_root.rstrip('/')

        return f"""# AGIX Knowledge (excluding defaults)
{agent_root}/knowledge/**
!{agent_root}/knowledge/default/**

# AGIX Instruments (excluding defaults)
...
"""
    ...
    def _get_agix_version(self) -> str:
        """Get current AGIX version"""
        try:
    ...
    def _translate_patterns(self, patterns: List[str], backup_metadata: Dict[str, Any]) -> List[str]:
        ...
        Replaces the backed up AGIX root path with the current AGIX root path
        ...
        Args:
            ...
            backup_metadata: Backup metadata containing the original agix_root

        Returns:
            List of translated patterns for the current system
        """
        # Get the backed up AGIX root path from metadata
        environment_info = backup_metadata.get("environment_info", {})
        backed_up_agent_root = environment_info.get("agix_root", "")

        # Get current AGIX root path
        current_agent_root = self.agix_root
        ...
        for pattern in patterns:
            # Check if the pattern starts with the backed up AGIX root
        ...
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                metadata = {
                    ...
                    "agix_version": self.agix_version,
                    "timestamp": datetime.datetime.now().isoformat(),
        ...
        try:
            ...
            with zipfile.ZipFile(temp_file, 'r') as zipf:
                ...
                for archive_path in archive_files:
                    # Archive path is already the correct relative path (e.g., "agix/tmp/settings.json")
                    ...
                    # so that patterns like "/home/rafael/agix/data/**" can match files correctly
    ...
    def _translate_restore_path(self, archive_path: str, backup_metadata: Dict[str, Any]) -> str:
        ...
        # Check if the archive path starts with the backed up AGIX root
