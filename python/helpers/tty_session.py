...
import asyncio, logging, os, sys, platform, errno
...
class TTYSession:
    def __init__(self, cmd, *, cwd=None, env=None, encoding="utf-8", echo=False):
        ...
        self._proc = None
        self._buf = None  # Created lazily in start() to bind to correct event loop
    ...
    async def start(self):
        # Create Queue in the correct event loop context
    ...
    async def close(self):
        ...
        if hasattr(self, "_pump_task") and self._pump_task:
            ...
            try:
                ...
                logger.debug("[TTYSession] Pump task cancelled during close")
    ...
    async def _on_data():
        while child.isalive():
            try:
                ...
                if data:
                    reader.feed_data(data.encode('utf-8', errors='replace') if isinstance(data, str) else data)
            except EOFError:
                break
            except Exception:
