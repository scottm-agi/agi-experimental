...
Click to open a video to learn how to install AGIX:

[![Easy Installation guide](/docs/res/easy_ins_vid.png)](https://www.youtube.com/watch?v=w5v5Kjx51hs)

The following user guide provides instructions for installing and running AGIX using Docker, which is the primary runtime environment for the framework. For developers and contributors, we also provide instructions for setting up the [full development environment](#in-depth-guide-for-full-binaries-installation).
...
- Docker Desktop provides the runtime environment for AGIX, ensuring consistent behavior and security across platforms
...
- Note: AGIX also offers a Hacking Edition based on Kali linux with modified prompts for cybersecurity tasks. The setup is the same as the regular version, just use the agent0ai/agi-experimental:hacking image instead of agent0ai/agi-experimental.

2.1. Pull the AGIX Docker image:
- Search for `agent0ai/agi-experimental` in Docker Desktop
...
>
> ```bash
> docker pull agent0ai/agi-experimental
> ```

2.2. Create a data directory for persistence:
- Choose or create a directory on your machine where you want to store AGIX's data
- This can be any location you prefer (e.g., `C:/agi-experimental-data` or `/home/user/agi-experimental-data`)
- This directory will contain all your AGIX files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your AGIX settings

> [!TIP]
> Choose a location that's easy to access and backup. All your AGIX data 
> will be directly accessible in this directory.

2.3. Run the container:
- In Docker Desktop, go back to the "Images" tab
- Click the `Run` button next to the `agent0ai/agi-experimental` image
...
- Under "Volumes", configure:
  - Host path: Your chosen directory (e.g., `C:\agi-experimental-data`)
  - Container path: `/agix`
...
> Alternatively, run the following command in your terminal:
> ```bash
> docker run -p $PORT:80 -v /path/to/your/data:/agix agent0ai/agi-experimental
> ```
...
- The Web UI will open. AGIX is ready for configuration!

![docker ui](res/setup/6-docker-agix-running.png)

> [!TIP]
> You can also access the Web UI by clicking the ports right under the container ID in Docker Desktop.

> [!NOTE]
> After starting the container, you'll find all AGIX files in your chosen 
...
3. Configure AGIX
- Refer to the following sections for a full guide on how to configure AGIX.

## Settings Configuration
AGIX provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings"button with a gear icon in the sidebar.
...
Learn more about Remote Function Calls and their purpose [here](#7-configure-agi-experimental-rfc).

> [!IMPORTANT]
> Always keep your API keys and passwords secure.

# Choosing Your LLMs
The Settings page is the control center for selecting the Large Language Models (LLMs) that power AGIX.  You can choose different LLMs for different roles:
...
> mixed in the vector database. Note that this will DELETE ALL of AGIX's memory.
...
#### Selecting your model within AGIX
...
## Using AGIX on your mobile device
AGIX's Web UI is accessible from any device on your network through the Docker container:
...
> [!NOTE]
> If you're running AGIX directly on your system (legacy approach) instead of 
> using Docker, you'll need to configure the host manually in `run_ui.py` to run on all interfaces using `host="0.0.0.0"`.

For developers or users who need to run AGIX directly on their system,see the [In-Depth Guide for Full Binaries Installation](#in-depth-guide-for-full-binaries-installation).

# How to update AGIX

1. **If you come from the previous version of AGIX:**
- Your data is safely stored across various directories and files inside the AGIX folder.
- To update to the new Docker runtime version, you might want to backup the following files and directories:
  ...
  - `/tmp/settings.json` - Your AGIX settings
...
- Reach for the folder where you saved your data and copy it to the new AGIX folder set during the installation process.
- AGIX will automatically detect your saved data and use it across memory, knowledge, instruments, prompts and settings.

> [!IMPORTANT]
> If you have issues loading your settings, you can try to delete the `/tmp/settings.json` file and let AGIX generate a new one.
...
- Go to "Images" tab and remove the `agent0ai/agi-experimental` image or click the three dots to pull the difference and update the Docker image.
...
> your AGIX data directory (the chosen directory on your machine).
...
> docker stop agi-experimental
>
> # Remove the container (data is safe in the folder)
> docker rm agi-experimental
>
> # Remove the old image
> docker rmi agent0ai/agi-experimental
>
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the AGIX repository. 
...
> - If any changes are made to the requirements of the updated version, you have to execute this command inside the agix conda env after activating it:
> ```bash
> pip install -r requirements.txt

# In-Depth Guide for Full Binaries Installation
- AGIX is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make AGIX work with its full development environment.
...
2. You don't necessarily need API keys: AGIX can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running AGIX without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download AGIX
- You can clone the AGIX repository (https://github.com/agent0ai/agi-experimental) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the AGIX releases [here](https://github.com/agent0ai/agi-experimental/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14-u.png" alt="AGIX download" width="500"/>
<br><br>

3. Extract the downloaded archive where you want to have it. I will extract them to "agi-experimental" folder on my Desktop - "C:\agi-experimental" on Windows and "~/Desktop/agi-experimental" on macOS.
...
2. In the terminal, navigate to your AGIX folder using **"cd"** command. Replace the path with your actual AGIX folder path.
~~~
cd C:\agi-experimental
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="AGIX cd" height="100"/>
<img src="res/setup/image-16.png" alt="AGIX cd" height="100"/>
<br><br>

3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"agix"** - short for AGIX. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
conda create -n agix python=3.12 -y
~~~
...
conda activate agix
~~~
And you should see that the **(base)** on the left has changed to **(agix)**. This means that this terminal now uses the new **agix** virtual environment and all packages will be installed into this environment.
...
> "conda activate agix" again for that window.
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox AGIX.
- AGIX only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows AGIX to 
...
- AGIX needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/agix` volume to the location of your local AGIX folder.
- Run the Docker container following the instructions.

## 5. Run the local AGIX instance
Run the AGIX with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the AGIX interface.

## 6. Configure AGIX
Now we can configure AGIX - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure AGIX.

## 7. Configure AGIX RFC
AGIX needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A0 needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on AGIX.
...
### Conclusion
After following the instructions for your specific operating system, you should have AGIX successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents. 

If you encounter any issues during the installation process, please consult the [Troubleshooting section](troubleshooting.md) of this documentation or refer to the AGIX [Skool](https://www.skool.com/agi-experimental) or [Discord](https://discord.gg/B8KZKNsPpj) community for assistance.
