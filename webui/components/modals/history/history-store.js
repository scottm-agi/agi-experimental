...
const model = {
  ...
  async open() {
    ...
    this.tokenCount = 0;
    this.showPreview = false; // Reset preview on open
  ...
  async fetchHistory() {
    ...
    try {
      ...
      const response = await callJsonApi('/history_get', {
        context: contextId,
      ...
      // Initialize ACE editor if not in preview mode
      if (!this.showPreview) {
  ...
  get renderedMarkdown() {
    if (this.selectedFormat !== 'markdown' || !this.historyData) return '';
    try {
      return marked(this.historyData);
    } catch (e) {
      console.error("Markdown rendering error:", e);
      return `<div class="error">Markdown rendering error: ${e.message}</div>`;
    }
  },
  ...
  scheduleEditorInit() {
    ...
    window.requestAnimationFrame(() => {
      if (this.isLoading || this.error || this.showPreview) return;
  ...
  initEditor() {
    ...
    // Set mode based on selected format
    const mode = this.selectedFormat === 'json' ? "ace/mode/json" : "ace/mode/markdown";
  ...
  updateModalTitle() {
    window.requestAnimationFrame(() => {
      ...
      if (this.error) {
        ...
        title.textContent = `History (${fmtLabel}) ~${this.tokenCount} tokens`;
      }
    });
  },
