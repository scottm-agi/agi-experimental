...
def file_tree(
    ...
    *,
    ...
    def make_entry(entry: os.DirEntry, parent: _TreeEntry, level: int, item_type: Literal["file", "folder"]) -> _TreeEntry | None:
        try:
    ...
    while queue and not limit_reached:
        ...
        folder_entries = [e for folder in folders if (e := make_entry(folder, parent_node, level, "folder"))]
        file_entries = [e for file_entry in files if (e := make_entry(file_entry, parent_node, level, "file"))]
