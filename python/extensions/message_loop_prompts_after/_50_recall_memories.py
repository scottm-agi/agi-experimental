...
from python.helpers.errors import RepetitionException

logger = logging.getLogger("agix.recall_memories")
    ...
    async def search_memories(self, log_item: log.LogItem, loop_data: LoopData, **kwargs):
        ...
        if set["memory_recall_query_prep"]:
            try:
                ...
                query = query.strip()
                logger.info(
                    f"[{self.agent.agent_name}] Memory recall: query LLM took "
                    f"{time.monotonic() - t_llm:.1f}s"
                )
            except RepetitionException as e:
                # Transient LLM condition — model looped during query generation.
                # Not a system error; downgrade to info and skip this recall cycle.
                logger.info(
                    f"[{self.agent.agent_name}] Memory recall: utility model "
                    f"repetition detected ({e.model}), skipping this cycle"
                )
                query = ""
            except (TimeoutError, asyncio.TimeoutError) as e:
                # ITR-48: LLM API timeout — fail fast, don't waste 3 retries.
                # Memory recall is non-critical; skip this cycle silently.
                logger.warning(
                    f"[{self.agent.agent_name}] Memory recall: utility model "
                    f"timed out ({type(e).__name__}), skipping this cycle"
                )
                query = ""
            except Exception as e:
                # Check if this is a litellm Timeout wrapped in a generic exception
        ...
        logger.info(
            f"[{self.agent.agent_name}] Memory recall: DB init took "
            f"{time.monotonic() - t_db:.1f}s"
        )
        ...
        logger.info(
            f"[{self.agent.agent_name}] Memory recall: FAISS search took "
            f"{time.monotonic() - t_search:.1f}s ({len(memories)} mems, {len(solutions)} sols)"
        )
        ...
        if set["memory_recall_post_filter"]:
            ...
            try:
                ...
                # Transient LLM condition — model looped during post-filter.
                # Skip filtering; use all unfiltered results.
                logger.info(
                    f"[{self.agent.agent_name}] Memory recall: filter model "
                    f"repetition detected ({e.model}), using unfiltered results"
                )
                filter_inds = []
            except (TimeoutError, asyncio.TimeoutError) as e:
                # ITR-48: LLM API timeout — skip filtering, use unfiltered.
                logger.warning(
                    f"[{self.agent.agent_name}] Memory recall: filter model "
                    f"timed out ({type(e).__name__}), using unfiltered results"
                )
                filter_inds = []
            except Exception as e:
                if "Timeout" in type(e).__name__ or "timeout" in str(e).lower():
        ...
        logger.info(
            f"[{self.agent.agent_name}] Memory recall: total pipeline took "
