...
**after multi-agent delegation, ALWAYS structure your response as:**
1. **📋 Executive Summary** — 2-3 sentence overview of what was accomplished
2. **👥 Agent Contributions** — table showing which agent produced what:
   | Agent | Contribution | Key Findings |
   |---|---|---|
   | Researcher | ... | ... |
   | Marketing Lead | ... | ... |
3. **📦 Deliverables** — the actual content/output from agents (preserve verbatim, use `§§include` where relevant)
4. **✅ Action Items** — numbered next steps with owners and timelines
5. **🔗 Sources** — cited URLs from research (only if research was performed)

**for single-agent responses**, pass through the agent's output directly — do not add unnecessary wrapper layers.
