...
class MemorizeSolutions(Extension):
    ...
    async def memorize(self, loop_data: LoopData, log_item: LogItem, **kwargs):
      try:
        ...
        # [FIX RCA-240] Scope solution extraction to the CURRENT topic only.
        ...
        # have already been processed, so re-processing them is wasteful AND
        # causes context overflow errors (191K > 133K tokens observed).
        ...
        if current_topic and current_topic.messages:
            ...
            log_item.update(heading="No messages in current topic to extract solutions from.")
            return
        ...
        for solution in solutions:
          ...
          print(f"[memorize_solutions] Error: {err}", file=sys.stderr, flush=True)
          err_summary = str(e)[:200] if str(e) else type(e).__name__
          log_item.update(
              type="warning", heading=f"Memorize solutions extension error: {err_summary}", content=err
          )
