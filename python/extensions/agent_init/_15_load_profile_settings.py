...
class LoadProfileSettings(Extension):
    ...
    async def execute(self, **kwargs) -> None:
        ...
        if not settings.get_settings().get("agent_profiles_enabled", True):
            return
        ...
        if files.exists(settings_path):
            try:
                ...
                if isinstance(override_settings, dict):
                    ...
                    # CRITICAL: Preserve the current agent_profile in the override.
                    # Without this, webhook-created agents (e.g., multiagentdev) lose
                    # their profile when this extension calls initialize_agent() with
                    # profile-specific settings.json that doesn't contain agent_profile.
                    # The initialize_agent() call falls back to the GLOBAL default profile.
                    if "agent_profile" not in override_settings and self.agent.config.profile:
                        override_settings["agent_profile"] = self.agent.config.profile
