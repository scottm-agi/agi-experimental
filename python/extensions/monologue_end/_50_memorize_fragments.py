...
class MemorizeMemories(Extension):
    ...
    async def memorize(self, loop_data: LoopData, log_item: LogItem, **kwargs):
      try:
        ...
        # [FIX RCA-240] Scope memorization to the CURRENT topic only.
        # Previously: self.agent.concat_messages(self.agent.history) sent the
        # ENTIRE history (all bulks + topics + current) to the utility model.
        # For long-running agents (46+ topics, 89+ messages), this easily
        # exceeds context limits (191K > 133K tokens observed).
        # The monologue_end extension runs after each cycle — previous cycles
        # have already been memorized, so re-processing them is wasteful AND
        # causes context overflow errors.
        current_topic = self.agent.history.current
        if current_topic and current_topic.messages:
            msgs_text = current_topic.output_text(human_label="user", ai_label="assistant")
        else:
            log_item.update(heading="No messages in current topic to memorize.")
            return

        # Guard: If current topic alone is too large, truncate to fit utility model
        from python.helpers import tokens as tok_helper
        approx_tokens = tok_helper.approximate_tokens(system + msgs_text)
        MAX_MEMORIZE_TOKENS = 100000  # Conservative limit for utility model
        if approx_tokens > MAX_MEMORIZE_TOKENS:
            # Keep proportional amount from end (most recent = most relevant)
            ratio = MAX_MEMORIZE_TOKENS / max(1, approx_tokens)
            keep_chars = int(len(msgs_text) * ratio)
            msgs_text = "... [earlier messages truncated for context limits] ...\n\n" + msgs_text[-keep_chars:]
        ...
        for memory in memories:
          ...
          print(f"[memorize_fragments] Error: {err}", file=sys.stderr, flush=True)
          err_summary = str(e)[:200] if str(e) else type(e).__name__
          log_item.update(
              type="warning", heading=f"Memorize memories extension error: {err_summary}", content=err
          )
