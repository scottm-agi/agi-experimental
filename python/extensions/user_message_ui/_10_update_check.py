...
# check for newer versions of AGIX available and send notification
...
class UpdateCheck(Extension):
    ...
    def send_notification(self, notif):
        ...
        notifs.send_notification(
            ...
            message=notif.get("message", "A newer version of AGIX is available. Please update to the latest version."),
            type=notif.get("type", "info"),
            detail=notif.get("detail", ""),
            display_time=notif.get("display_time", 15),
            group=notif.get("group", "update_check"),
            priority=notif.get("priority", notification.NotificationPriority.HIGH),
        )
