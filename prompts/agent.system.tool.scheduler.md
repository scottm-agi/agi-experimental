...
The task scheduler is a part of AGIX enabling the system to execute
...
### Task Scope Guidelines (MANDATORY)
Every task you create MUST have a clear `scope` definition. The scope describes what the task is allowed to do and — critically — what it is NOT allowed to do. This prevents task drift where a data-collection task accidentally starts modifying source code.

**Good scope examples:**
- `"Data collection and reporting ONLY. Do NOT modify source code, create PRs, or alter project files."`
- `"Use the growth_tip_generator tool to generate tips. Read-only access to data."`
- `"Code generation and testing for the X project. Full read/write access to /usr/projects/X/."`
- `"Run system_dashboard tool and report metrics. No file modifications."`

**Bad scope examples (too vague):**
- `"Do whatever is needed"` — too broad, will cause drift
- _(no scope at all)_ — task has no boundaries
...
* scope: str (RECOMMENDED) - Defines what the task is allowed and NOT allowed to do. Injected as mandatory boundaries at runtime. See Task Scope Guidelines above.
...
~~~json
{
    "thoughts": [
        ...
        "I must define a clear scope to prevent task drift"
    ],
    ...
    "tool_args": {
        ...
        "scope": "Send emails via SMTP ONLY. Do NOT modify source code or project files."
    }
}
~~~
...
* attachments: list[str] - Here you can add message attachments, valid are filesystem paths and internet urls
* dedicated_context: bool - if false, then the task will run in the context it was created in. If true, the task will have it's own context. If unspecified then false is assumed. The tasks run in the context they were created in by default.
* scope: str (RECOMMENDED) - Defines what the task is allowed and NOT allowed to do. See Task Scope Guidelines above.
...
~~~
#### scheduler:fork_task
Mark a task to be "forked". This creates a copy of the task's context and prepares it for a new run, often used to spawn a sub-task or an expert analysis task.
If no 'uuid' is provided, it attempts to fork the currently running task context.

##### Arguments:
* uuid: (Optional) string - The uuid of the task to fork.
* reason: (Optional) string - The reason for forking.
* summary: (Optional) string - A summary of why the task is being forked.

##### Usage:
~~~json
{
    "thoughts": [
        "I need to fork the expert responder for this issue"
    ],
    "headline": "Forking task for expert analysis",
    "tool_name": "scheduler:fork_task",
    "tool_args": {
        "uuid": "forgejo-expert-v1",
        "reason": "Issue #284",
        "summary": "Requesting Rule 11 expert analysis"
    }
}
~~~
