...
console.log("[COMPONENTS] File loaded. Location:", globalThis.location.href);
...
/**
 * Robustly identify the application base path (e.g. /agi/ or /)
 */
function getAppBase() {
  const pathname = globalThis.location.pathname;
  const result = (pathname.startsWith('/agi/') || pathname === '/agi') ? '/agi/' : '/';
  console.log(`[COMPONENTS] getAppBase() pathname="${pathname}" -> result="${result}"`);
  return result;
}
...
export async function importComponent(path, targetElement) {
  ...
  try {
    ...
    const componentUrl = appBase + (trimmedPath.startsWith("components/") ? trimmedPath : "components/" + trimmedPath);
    ...
    for (const node of allNodes) {
      if (node.nodeName === "SCRIPT") {
        const isModule =
          ...
          (node.textContent && node.textContent.includes("export "));

        if (isModule) {
          if (node.src) {
            // For <script type="module" src="..." use dynamic import
            // Resolve relative to the component's URL or app base
            const resolvedUrl = new URL(node.src, new URL(componentUrl, globalThis.location.origin)).toString();
            ...
            if (!componentCache[virtualUrl]) {
              // Calculate the base URL for this component to resolve relative imports
              // Prepend getAppBase() to ensure correct path resolution under /agi/
              const appBase = getAppBase();
              const fullComponentPath = componentUrl.startsWith('/') ? componentUrl : (appBase + componentUrl);
              const componentBaseUrl = new URL(fullComponentPath, globalThis.location.origin).href;

              // Transform relative import paths to absolute URLs
              // Enhanced regex to handle: import './xxx', import {x} from './xxx', etc.
              let content = node.textContent.replace(
                ...
                (match, bindings, importPath) => {
                  ...
                  if (importPath.startsWith("/")) {
                    // Absolute path: resolve against app base (e.g. /agi/ or /)
                    // If it already starts with the app base, use it, otherwise prepend it
                    const subPath = getAppBase();
                    const cleanImport = importPath.startsWith(subPath) ? importPath : (subPath + importPath.replace(/^\/+/, ""));
                    absoluteUrl = new URL(cleanImport, globalThis.location.origin).href;
                  } else {
                    // Relative path: resolve against the component's own absolute URL
                    absoluteUrl = new URL(importPath, componentBaseUrl).href;
                  }

                  // console.log(`[COMPONENTS] Transform: "${importPath}" -> "${absoluteUrl}"`);
                  return `import ${bindings} from "${absoluteUrl}"`;
                }
              );

              // Debug: log transformations
              console.log(`[COMPONENTS] Creating blob for ${virtualUrl}`);
              console.log(`[COMPONENTS] Full content:\n`, content);
              ...
              const modulePromise = import(blobUrl)
                .catch((err) => {
                  ...
                  console.error("Virtual URL:", virtualUrl);
                  console.error("Transformed content was:", content);
                  throw err;
                })
