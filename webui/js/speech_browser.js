// LAZY LOAD: Transformers.js is loaded only when microphone is first used
...
// Lazy loader for transformers module
async function getTransformers() {
    if (!transformersModule) {
        console.log('[Speech] Loading Transformers.js (first microphone use)...');
        transformersModule = await import('./transformers@3.0.2.js');
        console.log('[Speech] Transformers.js loaded successfully');
    }
...
class MicrophoneInput {
    ...
    async initialize() {
        try {
            // Lazy load transformers only when microphone is first used
    ...
    async process() {
        ...
        try {
            // Lazy load read_audio from transformers module
            const { read_audio } = await getTransformers();
