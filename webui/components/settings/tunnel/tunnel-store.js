...
const model = {
  ...
  dropdownOpen: false,
  providers: [
    { id: "cloudflared", title: "Cloudflare" },
    { id: "serveo", title: "Serveo" },
  ],
    ...
    try {
      ...
      if (data.success && data.tunnel_url) {
        ...
        if (this.tunnelLink !== data.tunnel_url) {
          ...
          localStorage.setItem("agix_tunnel_url", data.tunnel_url);
        }
        ...
        const storedTunnelUrl = localStorage.getItem("agix_tunnel_url");
        ...
        if (storedTunnelUrl) {
          ...
          if (verifyData.success && verifyData.is_valid) {
            ...
            // Clear stale URL
            localStorage.removeItem("agix_tunnel_url");
            this.tunnelLink = "";
  ...
  async generateLink() {
    ...
    try {
      ...
      if (!hasAuth) {
        const proceed = await Alpine.store('confirmation').confirm(
          'No authentication is configured. Creating a public tunnel without authentication means anyone with the URL can access your AGIX instance. Proceed anyway?',
          { target: document.activeElement, title: '⚠️ Security Warning' }
        );
